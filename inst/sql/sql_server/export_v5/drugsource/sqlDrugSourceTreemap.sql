select 	concept_hierarchy.concept_id,
  ...
  isnull(concept_hierarchy.sourcevoc_concept_code,'NA') + '||' +
	concept_hierarchy.sourcevoc_concept_name concept_path, 
...
from (select cast(stratum_1 as int) stratum_1, count_value from @results_database_schema.ACHILLES_results where analysis_id = 9700 GROUP BY analysis_id, stratum_1, count_value) ar1
	inner join
	(select cast(stratum_1 as int) stratum_1, count_value from @results_database_schema.ACHILLES_results where analysis_id = 9701 GROUP BY analysis_id, stratum_1, count_value) ar2
	on ar1.stratum_1 = ar2.stratum_1
	inner join
	(
		select sourcevoc.concept_id, 
			sourcevoc.concept_name as sourcevoc_concept_name,
			sourcevoc.sourcevoc_concept_code, 
		...
		select c1.concept_id, 
			c1.concept_name, 
			c2.concept_id as sourcevoc_concept_id, 
			c2.concept_code as sourcevoc_concept_code  -- concept_name
		from @vocab_database_schema.concept c1
			...
			and c2.concept_class_id = 'ATC 5th' -- Ingredient
		) sourcevoc
		left join
			(select c1.concept_id as sourcevoc_concept_id, max(c2.concept_id) as atc5_concept_id
			from
			...
			on c1.concept_id = ca1.descendant_concept_id
			and c1.domain_id = 'Drug'
			and c1.concept_class_id = 'ATC 5th'  -- Ingredient
			inner join 
			...
			) rxnorm_to_atc5 on sourcevoc.sourcevoc_concept_id = rxnorm_to_atc5.sourcevoc_concept_id
